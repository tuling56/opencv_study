/*
 *功能：粒子滤波用于目标跟踪
 *来自：http://www.cnblogs.com/tornadomeet/archive/2012/06/23/2559193.html
 *Info:[11/10/2014 jmy]
 */
...
void onMouse(int event,int x,int y,int,void*)
{
    ...
    if(event==CV_EVENT_LBUTTONDOWN)
    {
        ...
        select_rect=Rect(x,y,0,0);//这里一定要初始化，因为在opencv中Rect矩形框类内的点是包含左上角那个点的，但是不含右下角那个点。
    }
...
int partical_main(int argc, unsigned char* argv[])
{
    char c;
    ...
    while(1)
    {
        ...
        if(tracking)
        {
            ...
            {
                /****计算目标模板的直方图特征****/
                target_img=Mat(hsv,select_rect);//在此之前先定义好target_img,然后这样赋值也行，要学会Mat的这个操作
                ...
                {
                    pParticle->x=cvRound(select_rect.x+0.5*select_rect.width);//选定目标矩形框中心为初始粒子窗口中心
