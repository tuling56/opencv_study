...
int contoursg1()
{
    ...
    cvFindContours( img32s, storage, &contours, sizeof(CvContour),CV_RETR_CCOMP, CV_CHAIN_APPROX_SIMPLE, cvPoint(0,0) );
//  cvFindContours( img, storage, &contours, sizeof(CvContour),CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE, cvPoint(0,0) );
    ...
    cvCreateTrackbar( "levels+3", "contours", &g1_levels, 7, on_trackbar );
    {
        CvRNG rng = cvRNG(-1);
