...
*
* File:	contours_g2.cpp
* Brief: 存在轮廓等级的
* Source:OpenCV2.4.10官网教程
...
int contoursg2( int argc, char**)
{
        ...
        if( i == 0 )
        {
            for( int j = 0; j <= 10; j++ )
            {
                ...
                line(img, Point(cvRound(dx+100+j*10-80*cos(angle)),cvRound(dy+100-90*sin(angle))),
                    Point(cvRound(dx+100+j*10-30*cos(angle)),cvRound(dy+100-30*sin(angle))), white, 1, 8, 0);
            }
        }
    ...
    createTrackbar( "levels+3", "contours", &g2_levels, 7, on_trackbar );
